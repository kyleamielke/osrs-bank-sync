...
> **NOTE:** Use `net.runelite.api.gameval.InterfaceID.BANKMAIN` (top-level `int`) for group-id comparisons and `InterfaceID.Bankmain.<COMPONENT>` (nested class) for packed component IDs. The `net.runelite.api.widgets.InterfaceID` and `net.runelite.api.widgets.WidgetInfo` APIs are terminally deprecated and rejected by Plugin Hub CI.
...
| Bank interface group ID | `net.runelite.api.gameval.InterfaceID.BANKMAIN` | Integer group ID for the bank widget. |
...
| `InterfaceID.BANKMAIN` renamed | Very low | The integer ID is stable across cache updates; if the constant moves package, update the import path accordingly. |
