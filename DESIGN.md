...
- Shape: Use `ItemContainerChanged(BANK)` only for **dirty-tracking** (was-it-changed-since-last-submit). When the bank widget closes (`WidgetClosed.groupId == InterfaceID.BANKMAIN && isUnload()`), re-read the container fresh on the client thread, build a snapshot, and submit it. Additionally mount a "Sync" button on the bank widget (WikiSync `SyncButtonManager` pattern) that forces an immediate submission. As a fallback for "user logged out without closing the bank", a `GameStateChanged` handler submits any dirty cached snapshot on `HOPPING` / `CONNECTION_LOST` / `LOGIN_SCREEN`. Skip an automatic submission if the serialized payload (minus the per-submission `snapshot_id` and `captured_at` fields) is identical to the previous successful submission.
...
|---|---|---|
| Bank container ID | `net.runelite.api.gameval.InventoryID.BANK` | Used in `client.getItemContainer(InventoryID.BANK)`. |
| Bank interface group ID | `net.runelite.api.gameval.InterfaceID.BANKMAIN` | **Integer** group ID for the bank widget. The nested-class style `InterfaceID.Bankmain.ITEMS` is unrelated (child component IDs). Confirmed in `runelite-client` (`BankTagsPlugin`, `PotionStorage`, `WikiDpsManager`) and in `WidgetID.BANK_GROUP_ID = InterfaceID.BANKMAIN`. |
| Widget-closed unload check | `event.getGroupId() == InterfaceID.BANKMAIN && event.isUnload()` | Pattern lifted verbatim from `runelite/runelite` `TabInterface.java` and `PotionStorage.java`. `isUnload()` is necessary to avoid firing on intermediate widget reloads/redraws. |
...
| Item container changed | `net.runelite.api.events.ItemContainerChanged` with `event.getContainerId() == InventoryID.BANK` (the gameval constant is a `public static final int`; compare directly, no `.getId()` method exists on it) | For dirty-flag updates and `lastCapturedSnapshot` refresh. |
...
| `InterfaceID.BANKMAIN` renamed | Very low | The integer ID is stable across cache updates; if the constant moves package, we pin to `gameval` and update the import. |
